...
Teniendo ya la estructura del proyecto, se inicia el 21 de febrero la función de detección de ítems continuables se implementa en el módulo \texttt{utilization}, que se encarga de verificar la presencia de los SKUs configurados en CouchDB. Esta función se basa en las llamadas a la API de facturación para obtener los datos de utilización y compararlos con la lista de SKUs monitorizados. Para lograr la primera parte del requisito RNF04.02.
Para ello, se implementa la función \texttt{checkUtilization()} ---que se creó para PostMan y ya se explicó en la sección \hyperref[iteraciones:script-verificacion-postman]{Creación del script de verificación}--- además, como en PostMan, se realizará en dos fases: primero se detectará un SKU concreto y, una vez verificado su correcto funcionamiento, se extenderá al resto de SKUs.
