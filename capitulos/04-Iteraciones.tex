...
El requisito RF01.01 exige disponer de un usuario exclusivo para las pruebas, por lo que es necesario crear un contrato dedicado. Tras una breve investigación se concluyó que no existía ningún método sencillo para crearlo de forma automática, de modo que se optó por hacerlo manualmente, ya que el coste de tiempo era menor y el proceso más directo. La creación del contrato se inició el 15 de febrero y se finalizó el 19 de febrero de 2026.
...
Previamente a la creación del contrato, es necesario elaborar la lista de SKUs que se quieren detectar. Para ello se utilizó la documentación de IONOS Cloud~\cite{billing-IONOSCloud} y se creó el \hyperref[anexo:Lista-de-SKUs]{Anexo 1} con los SKUs preseleccionados, teniendo en cuenta los requisitos RF01.03 y RNF04, y cumpliendo el requisito RF01.02. Dicha lista se elaboró el 6 de febrero de 2026, respetando las fechas establecidas.

\vspace{0.5cm}

El primer problema encontrado fue vincular los recursos a sus SKUs, ya que la denominación de algunos de ellos no es intuitiva. Para resolverlo se recurrió a la API interna de IONOS Cloud~\cite{billing-IONOSCloud}.

Para utilizar dicha API se necesitaba, además de la documentación, un token generado por el contrato ---la forma de obtenerlo es sencilla y no es objeto del proyecto; para más información se puede consultar el \hyperref[anexo:crear-cuenta-token]{Anexo 2}---. Una vez obtenido el token, resulta imprescindible crear un plan de uso y control de estos tokens, dado que constituyen información delicada.
...
el plan de copias de seguridad en físico y la configuración inicial en CouchDB. Siendo estas tareas indispensables para cumplir tanto el requisito RF02.02,RF04.03 y RF05.01.  
\subsubsection{Despliegue del proyecto y vinculación a GitLab}
El proyecto se crea directamente desde GitLab ---cumpliendo así el requisito RNFO8--- debido a que se quiere integrar en un grupo de trabajo, concretamente en el equipo de \textit{Cloud Billing} y \textit{ERP} de IONOS Cloud, para que de ese modo, tras la finalización del proyecto, todo el equipo pueda seguir con su mantenimiento. Debido a esto, se sigue además la
