...
\begin{itemize}
    \item \textbf{existence-only:} Para recursos cuya cantidad puede variar legítimamente (almacenamiento consumido, tráfico de red, licencias por uso). En estos casos, basta con verificar que el SKU aparece en la respuesta de la API.
    
    \item \textbf{precise:} Para recursos con cantidades fijas y predecibles (cores de CPU, GB de RAM). Estos requieren comparar el valor reportado con el esperado, aplicando umbrales de tolerancia.
\end{itemize}

Por ejemplo, el SKU \texttt{C05000} (cores de CPU) tiene un valor esperado de 8 cores, mientras que \texttt{S01000} (almacenamiento HDD) solo requiere verificar su existencia, ya que el consumo real puede variar.
...
\subsubsection{Implementación de la verificación de cantidades}
Se ha creado la función \texttt{checkQuantities()} que extiende la verificación básica de existencia:

\begin{lstlisting}[caption=Función de verificación de cantidades, label=lst:check-quantities]
function checkQuantity(item, apiValue) {
    if (item.verification === 'existence-only') {
        return { status: 'ok', type: 'existence' };
    }
    
    const ratio = apiValue / item.value;
    const thresholds = item.thresholds;
    
    if (ratio < thresholds.error) {
        return { status: 'error', ratio, expected: item.value, actual: apiValue };
    } else if (ratio < thresholds.warning) {
        return { status: 'warning', ratio, expected: item.value, actual: apiValue };
    }
    return { status: 'ok', ratio };
}
\end{lstlisting}

La función calcula el ratio entre el valor real y el esperado, comparándolo con los umbrales configurados para determinar el nivel de alerta.

\subsubsection{Modificación del endpoint de utilización}
Se ha refactorizado el endpoint de verificación para eliminar el parámetro \texttt{:contractId} de la URL. El nuevo endpoint es:

\begin{verbatim}
GET /data/v1/utilization/check
\end{verbatim}

El \texttt{contractId} se obtiene ahora directamente desde la configuración del scheduler en CouchDB. Esto simplifica las llamadas y centraliza la gestión del contrato.
