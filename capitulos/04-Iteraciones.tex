...
\begin{itemize}
    ...
    \item \textbf{API RESTful nativa:} CouchDB expone una API HTTP que permite realizar operaciones
          ...
          el backend, simplificando la integración con nuestro proyecto.

    \item \textbf{Replicación maestro-maestro:} Soporta sincronización bidireccional entre múltiples
          instancias, lo que facilita la distribución de datos y la tolerancia a fallos. Siendo esto importante para cumplir con el requisito RNF01.
...
se puede consultar el \hyperref[anexo:crear-cuenta-token]{Anexo 2}. Además hay qye hacer la lista de SKUs que se quieren detectar, para lo que se ha usado la documentación de IONOS Cloud~\cite{billing-IONOSCloud} y se ha creado el \hyperref[anexo:Lista-de-SKUs]{Anexo 1} con los SKUs preseleccionados, dicha tarea se realizó el 6 de febrero de 2026, cumpliendo así con las fechas establecidas.
...
el plan de copias de seguridad en físico y la configuración inicial en CouchDB.
\subsubsection{Despliegue del proyecto y vinculación a GitLab}
El proyecto se crea directamente desde GitLab debido a que se quiere integrar en un grupo de trabajo, concretamente en el equipo de \textit{Cloud Billing} y \textit{ERP} de IONOS Cloud, para que de ese modo, tras la finalización del proyecto, todo el equipo pueda seguir con su mantenimiento. Debido a esto, se sigue además la
...
Debido a que el proyecto está integrado en el grupo de trabajo de \textit{Cloud Billing} y \textit{ERP}, se utiliza su grupo de bases de datos.
