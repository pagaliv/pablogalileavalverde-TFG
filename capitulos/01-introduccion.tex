...
\textbf{Nota:} A lo largo de la memoria se realizan referencias temporales a los hitos del proyecto. Dichos hitos y sus fechas límite se recogen en la tabla de la sección~\ref{sec:hitos}, dentro del capítulo de Planificación.
...
Como antecedentes se debe partir desde dos puntos de vista: tanto mis antecedentes como los de la empresa. Es importante destacar que \textbf{este proyecto no es una continuación de ningún trabajo previo}, sino un desarrollo completamente nuevo e independiente.
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Diseñar} una arquitectura de automatización de tests que permita la validación entre sistemas.
    ...
    \item \textbf{Evaluar} la efectividad del sistema propuesto en términos de rendimiento y reducción de errores.

    \item \textbf{Documentar} el desarrollo y los resultados obtenidos para su replicación o mejora.
\end{enumerate}
...
\section{¿Qué es el Diseñador de Centros de Datos (DCD)?}\label{sec:dcd-ionos}
...
Es muy intuitivo y tiene una interfaz bien desarrollada como se puede observar en la imagen. (Ver Figrua 1.1)

\begin{figure}[htb]
    \begin{center}
        \vspace*{0.3in}
        \includegraphics[width=9cm]{./imagenes/interfaz-DCD-1.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=8cm]{./imagenes/interfaz-DCD-2.png}
...
\begin{enumerate}
    ...
    \item \textbf{ Almacenamiento y Copias de Seguridad:} Son pilares complementarios de la estrategia de datos, siendo el Almacenamiento donde ``viven'' los datos actualmente y las copias de seguridad la manera de recuperar dicha información. Ofrece cinco servicios:
          \begin{itemize}
    ...
    \item \textbf{Servicios de red: } Define cómo se conectan y comunican todos los recursos, es una de las partes más importantes, desglosándose en varias partes:
          \begin{itemize}
              ...
              \item \textbf{Puerta de Enlace VPN: }  Servicio que crea un túnel cifrado y seguro.
              \item \textbf{Red de Distribución de Contenidos: } Una red global de servidores que almacenan en caché copias del contenido estático (CSS, JS, vídeos) cerca de los usuarios finales.
...
\paragraph{Facturación de los servicios}
Como antecedente de una explicación más detallada posteriormente, es improtante destacar que la facturación de los servicios es mensual y se divide en dos grandes grupos:
\begin{itemize}
    \item \textbf{Facturación de items continuables}: Son aquellos cargos que se generan de forma continua durante el mes, como el consumo de CPU, RAM o almacenamiento. Estos cargos no cambian cada mes a menos que se modifiquen manualmente.
    \item \textbf{Facturación de items no continuables}: Son aquellos cargos que se generan de forma puntual o condicional, como el trafico de red o el uso de tokens de inteligencia artificial.    
\end{itemize}
...
En el contexto de este proyecto, la API REST permite ejecutar y gestionar los tests de validación de facturación, comunicándose
...
La empresa ha solicitado que el sistema de validación se implemente como una API Canary. El término \textbf{``Canary''} proviene de la práctica minera de usar canarios para detectar gases peligrosos: del mismo modo que estos alertaban a los mineros ante condiciones peligrosas, una API Canary actúa como sistema de alerta temprana en el software. Una \textbf{API Canary} se define como:
...
El flujo de trabajo completo seguido en este proyecto para el desarrollo y despliegue de la API Canary puede consultarse en el \hyperref[anexo:flujo-canary]{Anexo~3}.
