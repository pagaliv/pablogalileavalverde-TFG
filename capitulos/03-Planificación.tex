...
Hechos ya los requisitos y sabiendo la metodología que se va a seguir, el siguiente paso es planificar de forma concreta el proyecto. En esta fase se crearán los distintos módulos del proyecto (EDT). Además, se establecerán las fechas límite para terminar los diferentes entregables y el tiempo estimado para cada apartado. Además, se incluirá un diagrama de hitos para verlo de forma más visual. Finalmente, se diseñará un plan de gestión de riesgos basándose en la metodología aprendida durante la carrera para evitar o mitigar posibles problemas.
...
En este apartado se describirán los diferentes módulos del EDT.
\begin{enumerate}
    ...
    \item \textbf{Análisis}
          \begin{enumerate}
              \item \textbf{Análisis de Requisitos :}  Identificación, documentación y validación de las necesidades del proyecto para definir las funcionalidades (funcionales) y restricciones (no funcionales) de un sistema.
                    ...
                    \end{enumerate}
              \item \textbf{Lista de items continuables: }Una lista con todos los SKUs de los recursos del DCD que están en funcionamiento actualmente. Se omiten los recursos que están obsoletos.
          \end{enumerate}

    \item \textbf{Gestión}
          \setcounter{enumii}{0} % Reiniciar contador del segundo nivel
          \begin{enumerate}
              \item \textbf{Planificación: } Es el proceso de definir objetivos, tareas, recursos, costos y cronogramas para guiar la ejecución y asegurar el éxito.
              \item  \textbf{Seguimiento y Control: }Tras la implementación y la finalización del proyecto se hace un seguimiento de su correcto funcionamiento.
              \item \textbf{Tecnologías y Herramientas: } Son las metodologías y enfoques que se han usado para la elaboración del proyecto.
    ...
    \item \textbf{Iteraciones}
          \setcounter{enumii}{0} % Reiniciar contador del segundo nivel
          \begin{enumerate}
              \item \textbf{Iteración 1:} Siendo esta la primera iteración, es en la que se crea la base en la que se basarán el resto de iteraciones, por lo que se deben tomar sus decisiones teniendo en cuenta que le podría afectar al resto del proceso. En esta primera iteración se detectaran los elementos que forman un contrato.
                    \setcounter{enumiii}{0} % Reiniciar contador del tercer nivel si es necesario
                    \begin{enumerate}
                        \item \textbf{Detección de items continuables: }La funcionalidad añadida en esta primera iteración se basa en lograr detectar que SKUs  forman parte de la lista creada, como el contrato será creado por nosotros se añadirán todos los SKUs, así que todos deberán estar, todo el que falte será un falso positivo.
                        \item \textbf{Creación de contrato completo:} De manera manual se crea un contrato con todos los SKUs de la lista.
                        \item \textbf{Configuración de Credenciales: } Esto se hace en todas las iteraciones, por lo que no se explicará en el resto, solo en este, se basa en la configuración concreta de CouchDB para el correcto funcionamiento.
              ...
              \item \textbf{Iteración 4:} En esta iteración se pasa a los items no continuables por lo que se estudia como hacer un gasto relativamente constante de estos.
                    \setcounter{enumiii}{0} % Reiniciar contador del tercer nivel si es necesario
                    \begin{enumerate}
                        ...
                        \item \textbf{Creación de gasto de tokens automático} Se estudia como lograr un gasto lo más constante posible de tokens.
                    \end{enumerate}
...
\begin{table}[h!]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        \hline
        \multirow{5}{*}{Iteración 3}         & Comprobación gasto items continables             & 16h    & \multirow{5}{*}{23h}  \\
        \cline{2-3}
                                             & Comprobación de que contrato completo            & 1h     &                       \\
        \cline{2-3}
                                             & Configuración de credenciales ,alertas y rutinas & 2h     &                       \\
        ...
        \cline{2-3}
                                             & Testeo de funcionamiento                         & 4h     &                       \\
        \hline
        \multirow{5}{*}{Iteración 5}         & Comprobación gasto items no continables          & 10h    & \multirow{5}{*}{21h}  \\
        \cline{2-3}
                                             & Comprobación de que contrato completo            & 1h     &                       \\
        \cline{2-3}
                                             & Configuración de credenciales, alertas y rutinas & 4h     &                       \\
...
\begin{table}[h!]
    ...
    \begin{tabular}{|c|l|c|c|c|c|c|c|c|c|c|c|c|c|} % <- segunda columna: l
        ...
        \hline
        TFG 3.3.1              & Comprobación gasto items continables         &                       &                       &                       &                       &                       &                       &                       & \cellcolor{ganttblue} & \cellcolor{ganttblue} &                       &                       & \hspace{0pt}          \\
        \hline
        TFG 3.4.1              & Comprobación de /usage items no continuables &                       &                       &                       &                       &                       &                       &                       &                       &                       & \cellcolor{ganttblue} & \cellcolor{ganttblue} & \hspace{0pt}          \\
        \hline
        TFG 3.4.2              & Creación de gasto de tockens automatico      &                       &                       &                       &                       &                       &                       &                       &                       & \cellcolor{ganttblue} & \cellcolor{ganttblue} &                       &                       \\
        \hline
        TFG 3.5.1              & Comprobación gasto items no continables      &                       &                       &                       &                       &                       &                       &                       &                       &                       &                       &                       & \cellcolor{ganttblue} \\
        \hline
    \end{tabular}
...
\begin{table}[h!]
    ...
    \begin{tabular}{|c|l|c|c|c|c|c|c|c|c|c|c|c|c|}
        ...
        \hline
        TFG 3.5.1              & Comprobación gasto items no continables & \cellcolor{ganttblue} & \cellcolor{ganttblue}                      &                       &                       &                       &                       &                       &                       &                       &     &     & \hspace{0pt} \\
        \hline
    \end{tabular}
    \caption{Cronograma de tareas del TFG}
\end{table}
...
\begin{table}[
    ...
    \begin{tabular}{|c|l|l|}
        ...
        \hline
        TFG 3.3.4                             & Testeo funcionamiento (2)                    & 29 de marzo de 2025   \\
        \hline
        TFG 3.4.5                             & Configuración de alertas y rutinas (2)       & 27 de marzo de 2026   \\
        \hline
        TFG 3.3.1                             & Comprobación gasto items continables         & 3 de abril de 2026    \\
        \hline
        ...
        \hline
        TFG 3.4.2                             & Creación de gasto de tockens automatico      & 20 de abril de 2026   \\
        \hline
        ...
        \hline
        TFG 3.5.1                             & Comprobación gasto items no continables      & 7 de mayo de 2026     \\
        \hline
